...
class VHDLExporter:
    ...
    def export(
        ...
        lines.append(f"    port (")
        lines.append(f"        clk   : in  STD_LOGIC;")
        lines.append(f"        rst_n : in  STD_LOGIC;")
        for inp in sorted(inputs):
            sanitized_inp = self._sanitize_name(inp)
            lines.append(f"        {sanitized_inp:6s}: in  STD_LOGIC;")
        if output_bits == 1:
            lines.append(f"        fsm_out : out STD_LOGIC")
        else:
            lines.append(
                f"        fsm_out : out STD_LOGIC_VECTOR({output_bits - 1} downto 0)"
            )
        lines.append(f"    );")
        ...
        if fsm_type == "moore":
            ...
            for state in states:
                ...
                if output_bits == 1:
                    lines.append(
                        f"            when {sanitized} => fsm_out <= '{output_val}';"
                    )
                else:
                    lines.append(
                        f'            when {sanitized} => fsm_out <= "{output_val}";'
            ...
            if output_bits == 1:
                lines.append(
                    f"            when others => fsm_out <= '{'0' * output_bits}';"
                )
            else:
                lines.append(
                    f'            when others => fsm_out <= "{"0" * output_bits}";'
            ...
            lines.append("    begin")
            if output_bits == 1:
                lines.append(f"        fsm_out <= '0'; -- default")
            else:
                lines.append(
                    f'        fsm_out <= "{"0" * output_bits}"; -- default'
                )
            lines.append("        case current_state is")
        ...
        lines.append("")
        lines.append(f"end architecture behavioral;")
        lines.append("")
    ...
    def _generate_next_state_logic(
        ...
        for state in states:
            ...
            for t in state_trans:
                ...
                if inp:
                    lines.append(
                        f"                if {self._sanitize_name(inp)} = '1' then"
                    )
                    lines.append(
                        f"                    next_state <= {to_state};"
                    )
